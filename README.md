...
[![UET module](https://github.com/HanGuTalker/uet-simulator/actions/workflows/uec.yml/badge.svg)](https://github.com/HanGuTalker/uet-simulator/actions/workflows/uec.yml)

`uet-simulator` is an ns-3 packet-level simulation framework for the Ultra
Ethernet Transport (UET) AI Base Profile. It models the UET wire headers and
endpoint behavior needed to study AI-oriented reliable transport, congestion
control, and collective communication on high-speed Ethernet fabrics.

The implementation includes:

- AI Base SES, PDS, and PDC behavior with network-order wire formats;
- reliable unordered (RUD), reliable ordered (ROD), and unreliable unordered
  (UUD) delivery;
...
The current UET test suite contains 25 passing protocol and end-to-end tests.
The verified requirement matrix is in
...
Ubuntu 22.04 or newer, including WSL2 Ubuntu, is recommended.
...
Run a representative 800 Gbps experiment after the build:
...
excluded from Git. Published measurements and reproduction commands are
recorded in the reports.

## Scope

This is a research simulator implementing the selected UEC AI Base Profile
requirements documented in the repository. It is not an official UEC
Consortium implementation or a conformance certification tool.

The repository publishes an ns-3.47 source snapshot and retains the official
ns-3 repository as the local `upstream` remote for future synchronization. It
uses the GPL-2.0-only license. The original ns-3 project documentation follows
below.
